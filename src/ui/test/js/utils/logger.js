...
export class logger {
    ...
    static smartStringify(obj, depth = 0, maxDepth = 2) {
        ...
        if (typeof obj === "function") {
            try {
                ...
                if (cleanFuncStr.length > maxFuncLength) {
                    return `[Function: ${obj.name || "anonymous"}] ${cleanFuncStr.substring(0, maxFuncLength)}...`;
                }
        ...
        if (obj instanceof Map) {
            const entries = Array.from(obj.entries()).map(([k, v]) => `${this.smartStringify(k, depth + 1, maxDepth)}: ${this.smartStringify(v, depth + 1, maxDepth)}`);
            return `Map(${obj.size}) {${entries.length ? " " + entries.join(", ") + " " : ""}}`;
        }
        // 處理陣列
        if (Array.isArray(obj)) {
            const items = obj.map((item) => this.smartStringify(item, depth + 1, maxDepth));
            return `[${items.join(", ")}]`;
        }
        // 處理其他物件
        try {
                    ...
                    try {
                        ...
                        if (typeof value === "function") {
                            try {
                                ...
                                if (cleanFuncStr.length > maxFuncLength) {
                                    result.push(`${prop}: [Function: ${value.name || "anonymous"}] ${cleanFuncStr.substring(0, maxFuncLength)}...`);
                                }
                                else {
                                    result.push(`${prop}: [Function: ${value.name || "anonymous"}] ${cleanFuncStr}`);
                                }
                            }
                            catch (e) {
                                result.push(`${prop}: [Function: ${value.name || "anonymous"}]`);
                            }
                        }
                        else {
                            result.push(`${prop}: ${this.smartStringify(value, depth + 1, maxDepth)}`);
                        }
                    }
                    catch (e) {
    ...
    static debug(...args) {
        ...
        console.log(`%c[DEBUG] ${new Date().toISOString()}: ${message}`, this.COLORS.DEBUG);
        pywebview.api.logger_debug(message);
    }
    static error(...args) {
        const message = this.formatMessage(...args);
        console.log(`%c[ERROR] ${new Date().toISOString()}: ${message}`, this.COLORS.ERROR);
        pywebview.api.logger_error(message);
    }
    static warning(...args) {
        const message = this.formatMessage(...args);
        console.log(`%c[WANING] ${new Date().toISOString()}: ${message}`, this.COLORS.WANING);
        pywebview.api.logger_warning(message);
    }
    static info(...args) {
        const message = this.formatMessage(...args);
        console.log(`%c[INFO] ${new Date().toISOString()}: ${message}`, this.COLORS.INFO);
