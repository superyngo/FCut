...
const updateHoverState = (event) => {
    ...
    // 找出新增的懸停元素
    const newHovered = hoveredElements.filter((el) => !_hoverState.hoveredElements.has(el));
    // 找出離開懸停的元素
    const leftHovered = Array.from(_hoverState.hoveredElements).filter((el) => !newHoveredSet.has(el));
    // 更新懸停集合
...
const updateMouseButtons = (event) => {
    ...
    if (prevLeft !== _mouseButtons.left) {
        logger.debug(`Left mouse button state changed: ${prevLeft ? "down" : "up"} -> ${_mouseButtons.left ? "down" : "up"}, buttons=${event.buttons}`);
    }
};
// 檢查事件是否符合過濾條件
...
const getElement = (target) => {
    if (!target)
        return null; // 修改這裡，返回 null 而不是 document.documentElement
...
const wrapCallback = (callbacks, config) => {
    if (config.throttle && config.throttle > 0) {
        return callbacks.map((callback) => throttle(callback, config.throttle, {
            leading: true,
            trailing: false,
        }));
    }
    else if (config.debounce && config.debounce > 0) {
        return callbacks.map((callback) => debounce(callback, config.debounce, {
            leading: true,
            trailing: false,
        }));
    }
...
const handleMouseEvent = (event) => {
    ...
    if (event instanceof MouseEvent) {
        ...
        if (event.type === MOUSE_EVENT_TYPE.Down) {
            ...
            logger.debug(`Mouse ${event.type}: buttons=${event.buttons}, left=${_mouseButtons.left}`);
        }
    }
    // 獲取事件類型並確保事件隊列初始化
    ...
    events.forEach((event) => {
        callbacks.forEach((registeredCallback) => {
            ...
            try {
                ...
                // 執行回調
                (_a = registeredCallback.wrappedCallbacks) === null || _a === void 0 ? void 0 : _a.forEach((wrappedCallback) => wrappedCallback(event));
                // 處理一次性回調
                ...
                logger.error(`執行鼠標事件回調 ID:${registeredCallback.id} 時發生錯誤: ${error}`);
            }
        });
    });
};
...
const startListening = (eventType, isPassive = false) => {
    ...
    logger.debug(`已為 ${eventType} 事件添加滑鼠全局監聽器 (passive: ${isPassive})`);
    // 啟動動畫循環 (如果未啟動)
...
export function onMousemove(callbackConfig) {
    // 添加類型屬性
    const configWithType = Object.assign(Object.assign({}, callbackConfig), { type: MOUSE_EVENT_TYPE.Move });
    // 添加監聽器
...
export function onClick(callbackConfig) {
    // 添加類型屬性
    const configWithType = Object.assign(Object.assign({}, callbackConfig), { type: MOUSE_EVENT_TYPE.Click });
    // 添加監聽器
    return addEventListeners(configWithType);
}
/**
...
export function onMousedown(callbackConfig) {
    // 添加類型屬性
    const configWithType = Object.assign(Object.assign({}, callbackConfig), { type: MOUSE_EVENT_TYPE.Down });
    // 添加監聽器
    return addEventListeners(configWithType);
}
/**
...
export function onMouseup(callbackConfig) {
    // 添加類型屬性
    const configWithType = Object.assign(Object.assign({}, callbackConfig), { type: MOUSE_EVENT_TYPE.Up });
    // 添加監聽器
    return addEventListeners(configWithType);
}
/**
...
export function onWheel(callbackConfig) {
    // 添加類型屬性
    const configWithType = Object.assign(Object.assign({}, callbackConfig), { type: MOUSE_EVENT_TYPE.Wheel });
    // 添加監聽器
    return addEventListeners(configWithType);
}
/**
...
export function onDrag(dragCallbackConfig) {
    ...
    const downHandle = onMousedown(Object.assign(Object.assign({}, dragCallbackConfig), { id: `drag_mousedown_${crypto.randomUUID()}`, callbacks: (event) => {
            dragState.isDragging = true;
            dragState.startX = event.clientX;
            dragState.startY = event.clientY;
            mousedown && mousedown(Object.assign(Object.assign({}, event), { dragState }));
        } }));
    // 鼠標移動處理
    const moveHandle = onMousemove(Object.assign(Object.assign({}, dragCallbackConfig), { target: null, id: `drag_mousemove_${crypto.randomUUID()}`, callbacks: (event) => {
            if (!dragState.isDragging)
                return;
            dragState.currentX = event.clientX;
            dragState.currentY = event.clientY;
            logger.debug(`Drag moving: ${event.clientX}, ${event.clientY}, delta: (${dragState.deltaX}, ${dragState.deltaY})`);
            mousemove(Object.assign(Object.assign({}, event), { dragState }));
        } }));
    // 鼠標釋放處理
    const upHandle = onMouseup(Object.assign(Object.assign({}, dragCallbackConfig), { target: null, id: `drag_mouseup_${crypto.randomUUID()}`, callbacks: (event) => {
            if (!dragState.isDragging)
                return;
            mouseup && mouseup(Object.assign(Object.assign({}, event), { dragState }));
            dragState.startX = 0;
            dragState.startY = 0;
            dragState.currentX = event.clientX;
            dragState.currentY = event.clientY;
            dragState.isDragging = false;
            logger.debug(`Drag ended: total movement (${dragState.deltaX}, ${dragState.deltaY})`);
        } }));
    // 合併所有監聽器的回調
...
export function setLowPowerMode(enable, frequency = 30) {
    ...
    logger.debug(`低功耗模式已${enable ? "啟用" : "停用"}，頻率: ${frequency}fps`);
}
/**
 * 創建虛擬鼠標事件 (用於測試)
 * @param type 事件類型
...
export function isHovering(element) {
    ...
    if (!targetElement)
        return false;
    return Array.from(_hoverState.hoveredElements).some((el) => el === targetElement || targetElement.contains(el));
}
/**
 * 獲取所有註冊的回調，按事件類型組織
 *
...
export function allCallbacks() {
    ...
    viewRegisteredCallbackConfigs().forEach((wrappedCallbackConfig) => {
        // 創建每個回調的淺拷貝並凍結數組
        const frozenCallbackConfig = Object.freeze(Object.assign({}, wrappedCallbackConfig));
...
function addEventListeners(callbackConfig) {
    ...
    const wrappedCallbackConfig = Object.assign(Object.assign({}, callbackConfig), { id: callbackId, type: eventType, wrappedCallbacks });
    // 添加到本次配置中
    registeredCallbacks.push(wrappedCallbackConfig);
    // 添加到全局配置中 - 現在以 ID 為鍵
    _callbackRegistry.registeredCallbackConfigs.set(callbackId, wrappedCallbackConfig);
    // 確定是否可以使用被動監聽器
    // 當 passive 選項明確設為 true 或者不需要 preventDefault 且 passive 未明確設為 false 時，使用被動監聽器
    const usePassiveListener = callbackConfig.passive === true ||
    ...
    logger.debug(`已註冊 ${eventType} 事件監聽器 (ID: ${callbackId}, passive: ${usePassiveListener})`);
    rebuildCache();
    // 返回清理函數
    return Object.assign(() => {
        // 移除所有註冊的回調
...
function removeRegisteredCallbackConfigs(registeredCallbacks) {
    ...
    registeredCallbacks.forEach((registeredCallback) => {
        // 遍歷並刪除每個回調
        if (_callbackRegistry.registeredCallbackConfigs.has(registeredCallback.id)) {
